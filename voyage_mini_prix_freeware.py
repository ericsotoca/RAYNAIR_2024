...
# Configuration des widgets DateEntry pour les dates de début et de fin
date_debut_defaut = datetime.now()
date_fin_defaut = datetime.now() + timedelta(days=30)  # Limite à 30 jours pour la version découverte


# Calculez la date d'aujourd'hui
aujourdhui = datetime.now()

# Calculez la date minimale (le lendemain)
mindate = aujourdhui + timedelta(days=1)

# Calculez la date maximale (30 jours plus tard)
maxdate = aujourdhui + timedelta(days=30)

# Configurez les widgets DateEntry pour les dates de début et de fin
# Utilisez mindate et maxdate pour restreindre la plage de sélection
entry_date_debut = DateEntry(window, width=12, background='lightblue', foreground='black', borderwidth=2, mindate=mindate, maxdate=maxdate, date_pattern='dd-mm-yyyy', state="readonly")
entry_date_fin = DateEntry(window, width=12, background='lightblue', foreground='black', borderwidth=2, mindate=mindate, maxdate=maxdate, date_pattern='dd-mm-yyyy', state="readonly")

# Positionnez les widgets DateEntry comme avant
entry_date_debut.grid(row=1, column=1, padx=10, pady=5, sticky="w")
entry_date_fin.grid(row=2, column=1, padx=10, pady=5, sticky="w")

# Insérez les valeurs par défaut dans les champs de saisie pour les dates
entry_date_debut.set_date(mindate.date())  # Fixez la date de début à mindate
entry_date_fin.set_date(maxdate.date())    # Fixez la date de fin à maxdate
...
# Fixer la date de la recherche à J+1 et désactiver l'entrée
entry_date_debut.delete(0, tk.END)
entry_date_debut.insert(0, mindate)
entry_date_debut.config(state='disabled')

# Fixer la date de la recherche à J+1 et désactiver l'entrée
entry_date_fin.delete(0, tk.END)
entry_date_fin.insert(0, maxdate)
...
# Fixer la durée du séjour à 4 jours et désactiver l'entrée
entry_duree_sejour.delete(0, tk.END)
entry_duree_sejour.insert(0, "4")
entry_duree_sejour.config(state='disabled')

# Fixer le prix max à 50€ et désactiver l'entrée
entry_prix_max.delete(0, tk.END)
entry_prix_max.insert(0, "50")
